...
Version: 2.7.2 - Fixed response validation for empty choices, null content, and null usage
"""
import asyncio
...
class AIClient:
        ...
        if self._client is None:
            if self.use_azure:
                ...
                self._client = AsyncOpenAI(
                    api_key=api_key, max_retries=0, timeout=float(AI_CLIENT_TIMEOUT)
                )
    ...
    async def review_code(
        ...
        if len(prompt) > MAX_PROMPT_LENGTH:
            logger.error(
                "prompt_too_large", length=len(prompt), max_length=MAX_PROMPT_LENGTH
            )
            raise ValueError(
        ...
        try:
            ...
            if not response.choices or len(response.choices) == 0:
                logger.error("ai_response_empty_choices", model=model)
                raise ValueError("AI response contained no choices")

            content = response.choices[0].message.content
            if content is None:
                logger.error("ai_response_content_null", model=model)
                raise ValueError("AI response content was null")

            # Handle missing usage object gracefully
            if response.usage is None:
                logger.warning("ai_response_missing_usage", model=model)
                tokens_used = 0
                prompt_tokens = 0
                completion_tokens = 0
            else:
            ...
            try:
                ...
                if "recommendation" not in result:
                    logger.error(
                        "ai_response_missing_recommendation", keys=list(result.keys())
                    )
                    raise ValueError(
                        "AI response missing required field: 'recommendation'"
                    )
                ...
                for idx, issue in enumerate(result["issues"]):
                    ...
                    for field in required_issue_fields:
                        if field not in issue:
                            ...
                            raise ValueError(
                                f"Issue at index {idx} missing required field: {field}"
                            )
            ...
            logger.error("openai_circuit_breaker_open", service="openai", model=model)
            # Re-raise original exception for proper upstream handling
            raise

        except openai.RateLimitError as e:
            retry_after = getattr(
                e, "retry_after", DEFAULT_CIRCUIT_BREAKER_TIMEOUT_SECONDS
            )
            logger.warning("openai_rate_limited", retry_after=retry_after, model=model)
            # Tenacity will retry automatically
            raise

        except openai.APITimeoutError as e:
            logger.warning("openai_timeout", model=model, timeout=AI_CLIENT_TIMEOUT)
            # Tenacity will retry automatically
            raise

        except openai.APIConnectionError as e:
            logger.warning("openai_connection_error", error=str(e))
            # Tenacity will retry automatically
            raise

        except asyncio.TimeoutError:
            # asyncio.wait_for timeout - convert to OpenAI timeout for consistent handling
            logger.error("ai_request_timeout", model=model, timeout=AI_REQUEST_TIMEOUT)
            raise openai.APITimeoutError(
                f"AI request exceeded timeout of {AI_REQUEST_TIMEOUT}s"
            )
    ...
    async def count_tokens(self, text: str, model: Optional[str] = None) -> int:
        ...
        try:
            ...
            logger.warning(
                "token_counting_failed", error=str(e), fallback_to_approximation=True
            )
            return len(text) // 4
